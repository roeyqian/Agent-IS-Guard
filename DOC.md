...
- 用户在浏览、加购、咨询 AI、接受干预、完成冷静小游戏、提交决策等阶段留下怎样的行为轨迹。

项目代码分为两个主要部分：

- `view/`：Vue 3 + Vite 前端界面，包含样本浏览、AI 聊天、压力测试、冷静小游戏、待购清单、决策提交和研究后台。
...
- 把短回合益智小游戏转化为非 AI 冷静任务，观察注意力切换是否帮助延迟冲动决策；
...
3. 将 BuyMate 干预策略事件化  
   前端提供需求反思、同类比较、话术重构、冷静延迟四类干预入口，并增加小恐龙跑酷、华容道、15 数码三类冷静小游戏。每次干预触发或小游戏完成都会记录到 `user_behaviors`，用于后续统计。
...
- RQ6：短回合益智小游戏是否能作为非 AI 干预，帮助参与者从促销刺激中暂停并降低即时加购倾向？
...
| 冷静小游戏 | `cooling_game` | 通过小恐龙跑酷、华容道、15 数码等短回合任务进行注意力切换 |
...
冷静小游戏同样复用 `intervention_check`，其中 `strategy = cooling_game`，并在 metadata 中记录 `game`、`action`、`score` 或 `moves`。这样无需新增数据库表即可观察小游戏打开、重开、结束和完成情况。
...
| `intervention_check` | 点击干预、完成冷静小游戏或结算前反思 | 观察理性干预触达 |
...
5. 干预触发  
   参与者点击需求反思、同类比较、话术重构或冷静延迟，系统打开守护型 AI 并记录干预事件；也可以进入冷静小游戏完成一次小恐龙跑酷、华容道或 15 数码任务，系统记录小游戏触达与完成情况。
